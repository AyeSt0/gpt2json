...
$setupExe = Join-Path $root "packaging\windows\build\GPT2JSON-CoreSetup-v$appVersion.exe"
$source = Join-Path $root "packaging\windows\artsetup\ArtSetup.cs"
$out = Join-Path $root "release\GPT2JSON-Setup-v$appVersion.exe"
...
Write-Host "输出安装器: $out"

$generatedSourceDir = Join-Path ([System.IO.Path]::GetTempPath()) ("GPT2JSON-Setup-Build-" + [guid]::NewGuid().ToString("N"))
...
if ($sourceText -notmatch [regex]::Escape("private const string AppVersion = `"$appVersion`";")) {
    throw "无法在安装器源码中注入显示版本 v$appVersion。"
}
if ($sourceText -notmatch [regex]::Escape('"GPT2JSON-CoreSetup-" + Version + ".exe"')) {
    throw "安装器外壳源码缺少按 Version 生成内嵌安装核心文件名的逻辑。"
}
