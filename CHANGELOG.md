...
- 协议链路加强：在密码验证和邮箱验证码提交前增加浏览器式页面预热；密码 / 验证码接口遇到疑似认证状态不同步时，会同会话刷新页面状态和 Sentinel 后自动修复一次，再决定是否进入账号级自动重试。
- 协议链路提速：Callback 换 JSON 超时会优先在当前会话内快速重试；验证码旧码 / 过期码会优先在当前验证码页重新取码并重提，减少从 OAuth 初始化开始整条链路重跑。
- 速度策略收紧：默认验证码等待从 180 秒降到 75 秒；旧码 / 取码超时只追加一次完整重试，不再进入 3+N 轮慢速补救。
